                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            @foreach($productVariables as $key=>$productVariable)
                                <tr>
                                    ...
                                    <th>
                                        @can('product_edit')
                                            <a class="btn btn-xs btn-info" href="{{ route('admin.products.editVariable', $productVariable->id) }}">
                                                {{ trans('global.edit') }}
                                            </a>
                                        @endcan
                                        @can('product_delete')
                                            <form action="{{ route('admin.products.variableDestroy', $productVariable->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
                            ...
                            @foreach($productImages as $key=>$productImage)
                                <tr>
                                    ...
                                    <th>
                                        ...
                                        @can('product_delete')
                                            <form action="{{ route('admin.products.imageDestroy', $productImage->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
                                                <input type="hidden" name="_method" value="DELETE">
                                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
...
<div class="modal fade show" id="modal-variable" style="display: none; padding-right: 16px;" aria-modal="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form action="{{ route("admin.products.addVariable") }}" method="POST" enctype="multipart/form-data">

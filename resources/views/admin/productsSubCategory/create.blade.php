...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route("admin.productsSubCategory.store") }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="row">
                ...
                <div class="form-group col-md-3">
                    ...
                    <select class="form-control" name="product_category_id" id="product_category_id" value="{{ old('is_active', isset($productSubCategory) ? $productSubCategory->product_category_id : 1) }}">
                        @foreach($productCategory as $key=> $parentCategory )
                            <option  value="{{$parentCategory->id}}">{{$parentCategory->product_category_name}} </option>
                        @endforeach
                    </select>
                </div>

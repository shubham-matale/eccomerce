    ...
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route("admin.language.create") }}">
                Add Language Translation Data
            </a>
            <a class="btn btn-success" href="{{ route("admin.file.download") }}">
                Generate Language Trnaslation File
            </a>
        </div>
    </div>
@endcan
                ...
                <tbody>
                    @foreach($languageData as $key => $language)
                        <tr data-entry-id="{{ $language->id }}">
                            ...
                            <td>
                                {{ $language->marathiText ?? '' }}
                            </td>
                            <td>

                                @can('product_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.language.edit', $language) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan
                                @can('product_delete')
